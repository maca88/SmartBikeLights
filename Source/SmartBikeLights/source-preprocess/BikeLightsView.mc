...
// #if touchScreen
  ...
  // #else
        ...
        return applyTapBehavior(lightData, tapBehavior);
  // #endif
    }
// #endif
...
// #if touchScreen
    ...
    private function initializeLightPanel(dc, lightData, position, width, height) {
        ...
        if (groupNameTextSize >= 0) {
            y += (dc.getFontHeight(groupNameTextSize) - StringHelper.getFontTopPadding(groupNameTextSize, fontTopPaddings) - dc.getFontDescent(groupNameTextSize)) + /* #include LIGHT_PANEL_MARGIN */ * 2;
            height -= y;
            groupName = [x + buttonGroupWidth / 2, -StringHelper.getFontTopPadding(groupNameTextSize, fontTopPaddings) + /* #include LIGHT_PANEL_MARGIN */, groupNameTextSize];
        }
        ...
        var fontResult = [0];
        var buttonPadding = /* #include LIGHT_PANEL_MARGIN */ * 2;
        ...
        for (i = 0; i < totalButtonGroups; i++) {
            ...
            for (var j = 0; j < totalButtons; j++) {
                ...
                var titleY = y + (buttonHeight - (titleList.size() * titleFontHeight) - titleFontTopPadding) / 2 + /* #include LIGHT_PANEL_MARGIN */;
        ...
        panelData[4] = x - (_batteryWidth / 2) - (/* #include LIGHT_PANEL_MARGIN */ / 2); // Light name x
        panelData[5] = y + ((footerHeight - (dc.getFontHeight(1) - StringHelper.getFontTopPadding(1, fontTopPaddings))) / 2); // Light name y
        panelData[6] = x + (lightNameWidth / 2) + (/* #include LIGHT_PANEL_MARGIN */ / 2); // Battery x
